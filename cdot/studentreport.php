...
<body>
    ...
    // Query the database for total number of results in the `user` table
    $stmt = $conn->prepare("SELECT COUNT(*) as total FROM users WHERE name LIKE ? OR contact LIKE ? OR email LIKE ?");
    $search_param = "%" . $search . "%";
...
// Assume $search_param, $offset, and $limit are already defined and sanitized

// Modify the SQL query to include a check for `type = 'User'`
$stmt = $conn->prepare("
    SELECT * 
    FROM users 
...
$result = $stmt->get_result();
    ...
    echo '<input class="form-control" id="search" name="search" type="search" placeholder="Search by Name, Contact, or Email" aria-label="Search" value="' . htmlspecialchars($search) . '">';
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<h2>No users found matching your search criteria.</h2>';
    }
