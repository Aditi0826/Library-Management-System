...
<body>
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) as total FROM books WHERE (title LIKE ? OR authors LIKE ? OR language_code LIKE ?) AND status = 'Available'");
    $search_param = "%" . $search . "%";
    ...
    $stmt = $conn->prepare("SELECT * FROM books WHERE (title LIKE ? OR authors LIKE ? OR language_code LIKE ?) AND status = 'Available' LIMIT ?, ?");
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<h2>Oops!! Looks like this book is not available at the moment</h2>';
    }
